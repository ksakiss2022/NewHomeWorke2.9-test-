...
public class TaskTwoTwo {
    ...
    private static final Map<Integer, String> mapFromTwo = new LinkedHashMap<>();
   // private static Map<Integer, String> mapToTwo = new HashMap<>();
...
//        var listTwenty = new java.util.ArrayList<>(List.of("Двадцать"));
        ...
        for (Map.Entry<Integer, String> pair : mapFromTwo.entrySet()) {
            System.out.println
                    (String.format("Key (numberKey) is: %s, Value (string) is : %s", pair.getKey(), pair.getValue()));
        }
    }
}
